...
namespace PurrNet.Lobby.Steam
{
    ...
    public class SteamLobbyProvider : LobbyProvider
    {
        ...
        /// <summary>Raised when the player accepts a Steam overlay invite; carries the lobby id to join.</summary>
        public event Action<string> onOverlayJoinRequested;
        ...
        public override Task Initialize()
        {
            if (SteamRuntime.EnsureInitialized())
            {
                ...
                _overlayJoinCallback = Callback<GameLobbyJoinRequested_t>.Create(evt =>
                    onOverlayJoinRequested?.Invoke(evt.m_steamIDLobby.m_SteamID.ToString()));
            }
